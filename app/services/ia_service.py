...
# FORZAR LOGS EN DOCKER: Enviar directo a consola (stdout)
...
def interpret_intent(user_text: str) -> dict:
    """
    Analiza el texto y extrae Categoria y Unidad automaticamente.
    """
    ...
    ACCIONES POSIBLES:
    1. VENTA: Salida de mercancia.
    2. COMPRA: Entrada de mercancia.
    ...
    6. LISTAR: Mostrar multiples productos segun un criterio (ubicacion, vencimiento, stock bajo, todos).
    7. DESCONOCIDO: Texto sin sentido comercial.

    REGLAS DE EXTRACCION:
    - "producto": Nombre limpio (ej: "Cemento Argos").
    - "precio": Numero entero sin simbolos (solo para CREAR).
    - "precio_compra": Costo de adquisicion o compra al proveedor (si se menciona).
    - "cantidad": Stock inicial o cantidad a operar (default 1).
    - "ubicacion": Lugar fisico de almacenamiento (ej: "Estante 1", "Cajon B", "Bodega").
    - "invima": Codigo de registro sanitario o INVIMA (ej: "2021M-0012345", "NSOC...").
    - "lote": Codigo de lote de produccion (ej: "L-452", "BATCH-01", "Lote 2024").

    REGLAS DE INFERENCIA (SOLO PARA ACCION 'CREAR'):
    - "categoria": Clasifica el producto logicamente.
       Ejemplos: Herramientas, Materiales, Electricos, Pinturas, Plomeria, Hogar.
    - "unidad": Infiere la unidad de medida estandar.
       - Cables, cuerdas, mangueras -> "MTS"
       - Cemento, yeso, cal -> "BULTO"
       - Pinturas, liquidos, quimicos -> "GALON" o "LITRO"
       - Pisos, enchapes -> "M2"
       - Si no es obvio -> "UND"
    ...
    - "Crea 50 metros de cable numero 12 a 1500" -> {"accion": "CREAR", "producto": "Cable No. 12", "precio": 1500, "cantidad": 50, "categoria": "Electricos", "unidad": "MTS"}
    ...
    - "Vendi 2 galones de thinner" -> {"accion": "VENTA", "producto": "thinner", "cantidad": 2}
    - "Cuanto vale el tubo pvc?" -> {"accion": "CONSULTA", "producto": "tubo pvc"}

    REGLAS PARA "ACTUALIZAR":
    - "producto": El nombre actual para buscarlo.
    - "nuevo_nombre": Solo si el usuario pide cambiar el nombre explicitamente.
    - "nuevo_sku": Si el usuario pide cambiar el codigo SKU o referencia.
    ...
    - Si el usuario dice "15 de mayo", asume el ano actual o el proximo logico.
    - Si el usuario dice "12/05/2027", conviertelo a "2027-05-12".
    - Si no menciona fecha, devuelve null o string vacio.

    EJEMPLOS:
    - "Crea Leche Colanta vence el 15/10/2026" -> {..., "fecha_vencimiento": "2026-10-15"}
    - "Crea Yogurt vence el 30 de diciembre" -> {..., "fecha_vencimiento": "2026-12-30"} (Calculando ano)
    """
