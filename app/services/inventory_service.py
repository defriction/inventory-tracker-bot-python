...
class InventoryService:
    def __init__(self, sheet_id: str):
        ...
        """
        logger.info(f"ðŸ”Œ Conectando a hoja de inventario ID: {sheet_id}")
    ...
    def process_instruction(self, intent: dict, user_name: str) -> str:
        ...
        """
        logger.info(f"ðŸ“© Procesando instrucciÃ³n de '{user_name}': {intent}")
        ...
        # ðŸ§  LOGICA DE RESPALDO: Si no hay nombre de producto, buscamos por SKU, Lote o Invima.
        if not product_name:
        ...
        if action == "DESCONOCIDO":
            ...
            return "ðŸ¤·â€â™‚ï¸ No entendÃ­ quÃ© quieres hacer\. Intenta: 'VendÃ­ 2 articulos'\."
            
        # --- ACCIÃ“N LISTAR (No requiere nombre de producto) ---
        if action == "LISTAR":
        ...
        if not product_name and action != "DESCONOCIDO":
            ...
            return "âŒ Necesito que me digas el nombre del producto\."

        try:
            # 1. BUSCAR EL PRODUCTO (LÃ³gica de Keywords)
            logger.info(f"ðŸ” Buscando producto: '{product_name}'")
            ...
            if row_idx:
                ...
                logger.info(f"âŒ Producto '{product_name}' no encontrado")
            ...
            # Para el resto de acciones el producto DEBE existir
                ...
                response_text = f"âŒ No encontrÃ© nada relacionado con '{self._escape(product_name)}' en tu inventario\."
            ...
            if response_text:
                logger.info(f"ðŸ“¤ Respuesta generada para Telegram: {response_text!r}")
                return response_text

        except Exception as e:
            logger.error(f"Error procesando accion: {e}")
            return "ðŸ’¥ OcurriÃ³ un error tÃ©cnico actualizando tu Excel\."
    ...
    def _find_product_row_by_keyword(self, query: str, exact_match: bool = False):
        ...
        """
        try:
            # OptimizaciÃ³n: Leer todo de una vez para buscar en mÃºltiples columnas (SKU, Nombre, Invima, Lote)
            rows = self.inventory_sheet.get_all_values()
            if not rows: return None, None
            
            query_norm = self._normalize(query)
            
            # FIX: Limpieza profunda de la bÃºsqueda (Query)
            # Quitamos prefijos comunes que la IA o el usuario podrÃ­an incluir
            ...
            # 1. BÃšSQUEDA POR IDENTIFICADORES (SKU, INVIMA, LOTE) - Prioridad MÃ¡xima
    ...
    def _create_product(self, name, price, initial_stock, user, category="General", unit="UND", expiration_date="", location="", purchase_price=0, invima="", lote="", requested_sku=""):
        ...
        logger.info(f"ðŸ†• Creando producto: {name} | Precio: {price} | Stock: {initial_stock}")
        ...
        exp_msg = f" \| ðŸ“… Vence: {self._escape(expiration_date)}" if expiration_date else ""
        
        # Construir bloque de detalles opcionales verticalmente
        details = []
        if location: details.append(f"ðŸ“ UbicaciÃ³n: {self._escape(location)}")
        if invima: details.append(f"ðŸ“ INVIMA: {self._escape(invima)}")
        if lote: details.append(f"ðŸ“¦ Lote: {self._escape(lote)}")
        
        details_msg = "\n".join(details)
        if details_msg: details_msg = "\n" + details_msg

        return (f"ðŸ†• *Producto Creado*\n"
                f"ðŸ“¦ {self._escape(name)}\n"
                f"ðŸ“‚ Cat: {self._escape(category)} \| ðŸ“ Unidad: {self._escape(unit)}\n"
                f"ðŸ’° Costo: ${self._escape(cost_val)}\n"
                f"ðŸ’² Precio: ${self._escape(price_val)}{exp_msg}"
                f"{details_msg}\n"
                f"ðŸ”¢ Stock inicial: {self._escape(initial_stock)}")

    def _handle_sale(self, row_idx, name, qty, user):
        """Procesa venta"""
        logger.info(f"ðŸ“‰ Procesando venta: {name} | Cantidad: {qty}")
        ...
        # Limpieza visual del SKU (para evitar .0 en nÃºmeros)
        if str(sku).endswith(".0"):
            sku = str(sku)[:-2]
            
        self._log_movement("VENTA", sku, name, -qty, user)

        return f"âœ… *Venta Registrada*\nðŸ”» {self._escape(name)} \(SKU: {self._escape(sku)}\)\nStock: {self._escape(current_stock)} âž¡ {self._escape(new_stock)}"

    def _handle_purchase(self, row_idx, name, qty, user):
        """Procesa compra"""
        logger.info(f"ðŸ“ˆ Procesando compra: {name} | Cantidad: {qty}")
        current_stock = int(self.inventory_sheet.cell(row_idx, 5).value or 0)
        ...
        return f"âœ… *Entrada Registrada*\nðŸŸ¢ {self._escape(name)} \(SKU: {self._escape(sku)}\)\nStock: {self._escape(current_stock)} âž¡ {self._escape(new_stock)}"
    ...
    def _handle_query(self, row_idx, name):
        ...
        logger.info(f"ðŸ” Datos crudos recuperados: {values}")
        
        # Ãndices de lista (Python empieza en 0): 
        ...
        logger.info(f"ðŸ“Š Datos parseados - SKU: {sku}, Stock: {stock}, Precio: {price}, Vence: {expiration}, Ubic: {location}, Invima: {invima}, Lote: {lote}")

        exp_msg = f"\nðŸ“… Vence: {self._escape(expiration)}" if expiration else ""
        loc_msg = f"\nðŸ“ UbicaciÃ³n: {self._escape(location)}" if location else ""
        invima_msg = f"\nðŸ“ INVIMA: {self._escape(invima)}" if invima else ""
        lote_msg = f"\nðŸ“¦ Lote: {self._escape(lote)}" if lote else ""

        return (f"ðŸ“¦ *Consulta de Inventario*\n"
                f"ðŸ“ Producto: {self._escape(name)}\n"
                f"ðŸ“‚ Cat: {self._escape(category)} \| ðŸ· SKU: {self._escape(sku)}\n"
                f"ðŸ”¢ Stock: {self._escape(stock)} {self._escape(unit)}\n"
                f"ðŸ’° Costo: ${self._escape(cost)}\n"
                f"ðŸ’² Precio: ${self._escape(price)}"
    ...
    def _update_product(self, row_idx, current_name, intent, user):
        ...
        logger.info(f"ðŸ”„ Actualizando producto: {current_name}")
    ...
    def _handle_list(self, intent):
        """Genera un reporte de productos segÃºn criterio"""
        criterio = intent.get('criterio')
        loc_filter = intent.get('ubicacion')
        
        logger.info(f"ðŸ“‹ Generando lista por criterio: {criterio}")
        
        # Obtenemos todos los datos (Cuidado con inventarios gigantes, para PyMEs estÃ¡ bien)
        rows = self.inventory_sheet.get_all_values()
        ...
        for i, row in enumerate(rows[1:]): # Saltamos header
            ...
            if criterio == "ubicacion":
                # BÃºsqueda parcial (ej: "Estante" coincide con "Estante 1")
                if loc_filter and self._normalize(loc_filter) in self._normalize(loc):
                    match = True
            
            elif criterio == "stock_bajo":
                # Umbral fijo de 5 unidades (podrÃ­a ser configurable)
                if stock <= 5:
                    match = True
                ...
                if exp_str:
                    try:
                        ...
                        # Listar vencidos o que venzan en prÃ³ximos 30 dÃ­as
                        if days_diff <= 30:
                            match = True
                    except:
                        pass # Fecha invÃ¡lida, ignorar
        ...
        if not results:
            return f"ðŸ” No encontrÃ© productos con el criterio: *{self._escape(criterio)}*\."
            
        # Limitamos a 200 para no saturar Telegram
        limit = 200
        display = results[:limit]
        msg = f"ðŸ“‹ *Reporte: {self._escape(criterio.upper())}*\n" + "\n".join(display)
        
        if len(results) > limit:
            msg += f"\n\n_\.\.\.y {len(results) - limit} mÃ¡s\._"
